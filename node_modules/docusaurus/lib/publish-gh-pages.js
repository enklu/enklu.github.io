...
shell.echo('Starting publish script...');
...
const PROJECT_NAME =
  ...
  siteConfig.projectName).replace(/ /g, '');
...
const CUSTOM_SSH = process.env.SSH_PATH;
...
let remoteBranch = `git@${GITHUB_HOST}:${ORGANIZATION_NAME}/${PROJECT_NAME}.git`;
...
const buildCmd = `node "${path.join(__dirname, 'build-files.js')}" ${process.argv.slice(2).join(' ')}`
shell.echo(`Building site: ${buildCmd}`);
if (
    shell.exec(buildCmd).code
) {
...
shell.echo('Cloning master...');
var gitClone = `git clone ${CUSTOM_SSH ? `-i ${CUSTOM_SSH} -o IdentitiesOnly=yes ` : ''}--depth 1 ${remoteBranch} ${PROJECT_NAME}-${DEPLOYMENT_BRANCH}`;
