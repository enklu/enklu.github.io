...
const GIT_USER = (process.env.GIT_USER || siteConfig.gitUser).trim();
...
const USE_SSH = process.env.USE_SSH || siteConfig.useSsh;
...
fs.copy(
  ...
  error => {
    ...
    const hash = shell.exec('git rev-parse --short HEAD').stdout.trim();

    const commitMessage = CUSTOM_COMMIT_MESSAGE || `Deploy from ${hash}`;
